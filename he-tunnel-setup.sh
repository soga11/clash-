...
# 提示用户准备隧道信息
echo -e "${CYAN}════════════════════════════════════════${NC}"
echo -e "${YELLOW}  准备 HE 隧道信息${NC}"
echo -e "${CYAN}════════════════════════════════════════${NC}"
echo ""
echo "请先登录 https://tunnelbroker.net/ 创建隧道"
echo "然后准备以下信息："
echo ""
echo "  1. Server IPv4 Address    (HE 服务器地址)"
...
echo ""
echo -e "${CYAN}════════════════════════════════════════${NC}"
echo -e "${YELLOW}  步骤 1/4: 本机 IPv4 地址${NC}"
echo -e "${CYAN}════════════════════════════════════════${NC}"
echo ""
...
if [[ -n "$OUTBOUND_IPV6" ]]; then
    ...
    if [[ -n "$GEO_INFO" ]]; then
        ...
        COUNTRY_CODE=$(echo "$GEO_INFO" | jq -r '.country' 2>/dev/null || echo "")
